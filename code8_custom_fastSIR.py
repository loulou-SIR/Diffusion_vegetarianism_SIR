...
def _process_trans_SIR_custom(time, G, source, target, times, S, I, R, Q, status,
...
def fast_nonMarkov_SIR_(G, trans_time_fxn=None,
    ...
    **full_data**  Simulation_Investigation object
    ...
    for u in initial_infecteds:
        ...
        Q.add(tmin, _process_trans_SIR_custom, args=(G, None, u, times, S, I, R, Q,
                                                    status, rec_time,
...
def fast_SIR_custom(G, tau, gamma, initial_infecteds = None, initial_recovereds = None,
...
beta=1
gamma=0.6
t1,S1,I1,R1= fast_SIR_custom(g, tau = beta, gamma=gamma, initial_infecteds=initial_vege, transmission_weight="weight", return_full_data=False)
plt.plot(t1,I1)
plt.show()

'''
def average_stochastic_EoN(sol_number, beta, gamma):
    max_t=0
    max_len=0
    simulations=[]
    for i in range(sol_number):
        ...
        t1,S1,I1,R1= EoN.fast_SIR(g, tau = beta, gamma=gamma, initial_infecteds=initial_vege, transmission_weight=None, return_full_data=False)
    ...
    return t_mean,S_mean,I_mean,R_mean

def average_stochastic_custom(sol_number, beta, gamma):
    max_t=0
    max_len=0
    simulations=[]
    state_ini=[]
    ...
    for i in range(sol_number):
        ...
        initial_vege=[v for v in g.nodes() if g.nodes[v]['vege']==1]
        state_ini.append(state_vege(g))#we store initial state of g concerning vegetarians
        freq_ini.append(frequency_vege(g))

        #Use of the fast SIR function of EoN module
        t1,S1,I1,R1= fast_SIR_custom(g, tau = beta, gamma=gamma, initial_infecteds=initial_vege, transmission_weight="weight", return_full_data=False)
        simulations.append([t1,S1,I1,R1])
        state_end.append(state_vege(g)) #we store end state of g
        freq_end.append(frequency_vege(g))
        if max(t1)>max_t:
            max_t=max(t1)
        if len(t1)>max_len:
            max_len=len(t1)
    ...
    for i in range(len(state_ini)):
        for key in state_ini_mean.keys():
            state_ini_mean[key]+=state_ini[i][key]
            state_end_mean[key]+=state_end[i][key]
            freq_ini_mean[key]+=freq_ini[i][key]
            freq_end_mean[key]+=freq_end[i][key]

    for key in state_ini_mean.keys():
        state_ini_mean[key]/=len(state_ini)
        state_end_mean[key]/=len(state_ini)
        freq_ini_mean[key]/=len(freq_ini)
        freq_end_mean[key]/=len(freq_ini)

    return t_mean,S_mean,I_mean,R_mean,state_ini_mean,state_end_mean, freq_ini_mean,freq_end_mean


##Execute functions

#test_graph=nx.watts_strogatz_graph(n=10000,k=4,p=0.3)
#initial_vege_test=[random.randint(1,9999) for i in range(500)]

beta1=1
gamma1=0.6
sol_number=50

#Execute custom fast SIR
t1,S1,I1,R1,state_ini,state_end, freq_ini, freq_end= average_stochastic_custom(sol_number, beta1,gamma1)

#Execute reference fast SIR
t2,S2,I2,R2= average_stochastic_EoN(sol_number, beta1,gamma1)

#only run this part once
##Export states of the network before and after running custom algorithm on it

with open('/Users/louisdretzolis/Desktop/Proyecto Python/state_network2.csv', 'w') as myfile:
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    wr.writerow(state_ini.values())
    wr.writerow(state_end.values())
    wr.writerow(freq_ini.values())
    wr.writerow(freq_end.values())
...
fig1.suptitle('Custom algorithm vs reference algorithm')
ax1 = fig1.add_subplot(111, facecolor='#dddddd', axisbelow=True)

#plot custom SIR
ax1.plot(t1, S1/N1, 'b', linestyle='-', alpha=1, lw=2, label='Custom : Omnivorous')
ax1.plot(t1, I1/N1, 'r', linestyle='-', alpha=1, lw=2, label='Custom : Vegetarian')
ax1.plot(t1, R1/N1, 'g', linestyle='-',alpha=1, lw=2, label='Custom : Ex-vegetarian')

#plot reference SIR
ax1.plot(t2, S2/N1, 'b', linestyle='--', alpha=0.65, lw=2, label='EoN : Omnivorous')
ax1.plot(t2, I2/N1, 'r', linestyle='--', alpha=0.65, lw=2, label='EoN : Vegetarian')
ax1.plot(t2, R2/N1, 'g', linestyle='--',alpha=0.65, lw=2, label='EoN : Ex-vegetarian')
