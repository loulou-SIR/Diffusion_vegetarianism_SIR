...
'''This code generates a small world network and plot the impact of one of its parameters k and p by fixing the other parameter
'''
...
for i in range(4):
    ...
    ax.plot(t_mean, I_mean/N, color, linestyle='-', alpha=1, lw=2, label='Transitioning for k = '+str(k))
...
for i in range(4):
    ...
    #style=linestyle_list[i]
    t_mean,S_mean,I_mean,R_mean=sim[0],sim[1],sim[2],sim[3]
    ax.plot(t_mean, I_mean/N, color, linestyle='-', alpha=1, lw=2, label='Transitioning for p = '+str(p))
